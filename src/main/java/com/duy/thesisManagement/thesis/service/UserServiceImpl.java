...
public class UserServiceImpl implements UserService {
	...
	@Override
		...
		public UserDTO updateUser(Integer id, UserUpdatingDTO userUpdatingDTO) {
				...
				if (foundUser.isPresent()) {
							...
							updatingUser.setXyz(userUpdatingDTO.getXyz());
	...
	public void changePassword(Integer id, String newPassword) {
		...
		if (userOpt.isPresent()) {
			...
			if (newEncodedPassword.equals(existingUser.getPassword().toString())) {
...
//			boolean matches = encoder.matches(newEncodedPassword,existingUser.getPassword());
//			if(matches){
//				throw new BadRequestException("You cannot change password with the same existing password. Please input the different value");
//			}
...
//		@Override
//		public void changePassword(Integer id, PasswordChangingDTO passwordChangingDTO) {
//				if (StringUtils.isBlank(passwordChangingDTO.getNewPassword())) {
//						throw new BadRequestException("New password cannot be blank");
//				}
//
//				Optional<User> userOpt = this.userRepository.findById(id);
//				if (userOpt.isPresent()) {
//						User existingUser = userOpt.get();
//						String oldPassWord = this.encoder.encode(passwordChangingDTO.getOldPassword());
//						String newEncodedPassword = this.encoder.encode(passwordChangingDTO.getNewPassword());
//						if(oldPassWord.compareTo(existingUser.getPassword()) != 0){
//							throw new BadRequestException("You cannot change password with the differences old password. Please input the different value");
//						}
