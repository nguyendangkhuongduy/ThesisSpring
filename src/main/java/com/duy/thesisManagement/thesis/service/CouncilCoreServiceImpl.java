...
public class CouncilCoreServiceImpl implements CouncilCoreService {
    ...
    @Override
    public void activeCouncil(Integer id) {
        Optional<Council> councilOpt = this.councilRepository.findById(id);
        if (councilOpt.isPresent()) {
            Council active = councilOpt.get();
            active.setActive(true);
            this.councilRepository.save(active);
            return;
        }
        throw new ResourceNotFoundException("Cannot find any council for active action with id: " + id);
    }
